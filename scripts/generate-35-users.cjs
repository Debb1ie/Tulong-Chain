 ...
 * CommonJS version - run from project root or frontend dir.
 */
...
async function main() {
  ...
  for (let i = 0; i < USER_COUNT; i++) {
    ...
    if (!funded) {
      console.log(`⚠️  Friendbot failed ${wallet.slice(0,8)}..., skipping`);
    ...
    try {
      ...
      console.log(`❌ ${(i+1).toString().padStart(2)}/${USER_COUNT} ${name} — ${e.message}`);
    }
  ...
  console.log("|---|---|---|---|---|---|---|");
  users.forEach(u => {
    const expl = (u.txHash.startsWith("PENDING") || u.txHash.includes("FAIL")) ? "N/A" : `[View](https://stellar.expert/explorer/testnet/tx/${u.txHash})`;
    const ns = u.name.length > 12 ? u.name.slice(0,10)+".." : u.name;
    const fb = u.feedback.replace(/"/g, "'").slice(0, 35);
    console.log(`| ${u.id} | \`${u.wallet}\` | ${ns} | ${u.rating}/5 | "${fb}..." | ${u.txHash.slice(0,12)}... | ${expl} |`);
  });
  console.log("\n");
}
