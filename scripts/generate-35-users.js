 ...
 * Creates accounts, funds via Friendbot, donates XLM to contract.
 * Output: scripts/test-users-output.json + console markdown table
 */
...
async function donateXlm(keypair, amountXlm) {
  ...
  const baseAmount = BigInt(Math.round(amountXlm * 1e7)); // stroops
...
async function main() {
  ...
  for (let i = 0; i < USER_COUNT; i++) {
    ...
    await new Promise(r => setTimeout(r, 2500)); // let XLM arrive

    // Step 2: Donate XLM to contract
    let tx = "PENDING";
    try {
      ...
      console.log(`❌ ${(i+1).toString().padStart(2)}/${USER_COUNT} ${name} — error: ${e.message}`);
    }
  ...
  console.log("\n📌 Next steps:");
  console.log("1. Verify a sample of tx hashes on Stellar Expert");
  console.log("2. Take screenshots of successful transactions");
  console.log("3. Copy table above → FEEDBACK.md (replace 5-user table)");
  console.log("4. Update README Verified Testnet Users count to 35+\n");
}
